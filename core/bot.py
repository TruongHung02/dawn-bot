...
class Bot(DawnExtensionAPI):
    ...
    async def process_reverify_email(self, link_sent: bool = False) -> OperationResult:
        ...
        try:
            ...
            redirect_url = await asyncio.to_thread(ByPassCloudFlare.get_redirect_url(url=confirm_url["data"])) 
            ...
            if token and key:
                ...
                logger.error(f"Account: {self.account_data.email} | Failed to pass confirm cloudflare")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if token and key:
                    ...
                    logger.error(
                        f"Account: {self.account_data.email} | Failed to bypass token cloudflare"
                    )
            else:
                logger.error(
